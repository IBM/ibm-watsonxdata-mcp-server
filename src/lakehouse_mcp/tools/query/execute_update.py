...
async def execute_update(
    ...
    while True:
        ...
        if state == "FAILED":
            ...
            raise RuntimeError(f"UPDATE query failed: {error_message}")

        if state == "CANCELED":
            raise RuntimeError("UPDATE query was canceled")

        if elapsed_time >= max_wait_time:
            raise TimeoutError(f"UPDATE query {query_id} did not complete within {max_wait_time} seconds")
        ...
        if state == "" and data.get("nextUri", "") == "":
            ...
            if empty_response_count >= 3:
                raise RuntimeError(f"UPDATE query execution failed: received multiple empty responses. Query ID: {query_id}")
        else:
            empty_response_count = 0
