"""
Create Prestissimo engine tool.

This tool creates a new Prestissimo engine in watsonx.data.
...
@mcp.tool()
async def create_prestissimo_engine(
    ctx: Context,
    origin: str,
    display_name: str,
    ...
    {
        "origin": "native",
        "display_name": "My-Prestissimo-Engine",
        "description": "Prestissimo engine for fast queries",
        "tags": [],
        "associated_catalogs": [],
        "configuration": {
            "size_config": "custom",
    ...
    Args:
        ...
        engine_id: (optional) Custom engine ID (must match pattern: prestissimo-0 to prestissimo-1000)
    ...
    if coordinator["quantity"] != 1:
        return {
            "error": True,
            "error_message": f"coordinator quantity must be 1 for Prestissimo, got {coordinator['quantity']}",
            "status_code": 400,
        }
    ...
    logger.info("creating_prestissimo_engine", display_name=display_name, origin=origin)

    path = "/v3/prestissimo_engines"
    response = await watsonx_client.post(path, body)

    # Check for API errors
    if response.get("error"):
        logger.error("create_prestissimo_engine_failed", error=response.get("error_message"))
        return {
            "error": True,
    ...
    logger.info("prestissimo_engine_created", engine_id=response.get("id"))
