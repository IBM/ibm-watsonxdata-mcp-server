"""
Scale Prestissimo engine tool.

This tool scales a Prestissimo engine by adjusting coordinator and worker node counts.
...
@mcp.tool()
async def scale_prestissimo_engine(
    ctx: Context,
    engine_id: str,
    ...
    Args:
        ...
        coordinator_quantity: Number of coordinator nodes (must be 1 for Prestissimo)
    ...
    if coordinator_quantity != 1:
        return {
            "error": True,
            "error_message": f"coordinator_quantity must be 1 for Prestissimo engines, got {coordinator_quantity}",
            "status_code": 400,
        }
    ...
    logger.info(
        "scaling_prestissimo_engine",
        engine_id=engine_id,
        coordinator=body.get("coordinator"),
        worker=body.get("worker"),
    )

    path = f"/v3/prestissimo_engines/{quote(engine_id, safe='')}/scale"
    response = await watsonx_client.post(path, body)

    # Check for API errors
    if response.get("error"):
        logger.error("scale_prestissimo_engine_failed", error=response.get("error_message"))
        return {
            "error": True,
